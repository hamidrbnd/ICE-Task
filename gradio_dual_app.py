...
# Item & User KNNs
# -------------------------------
knn_items = NearestNeighbors(n_neighbors=K_ITEM_DEFAULT, metric="cosine", algorithm="brute", n_jobs=-1).fit(ui_matrix.T)
knn_users = NearestNeighbors(n_neighbors=K_USER_DEFAULT, metric="cosine", algorithm="brute", n_jobs=-1).fit(ui_matrix)
...
with gr.Blocks(title="Book Recommender") as demo:
    ...
    with gr.Tab("🔮 For You (User-ID)"):
        ...
        topN = gr.Slider(5, 30, value=TOPN_DEFAULT, step=1, label="Top-N")
        K_item = gr.Slider(10, 200, value=K_ITEM_DEFAULT, step=10, label="Neighbors per item (K_item)")
        useH = gr.Checkbox(value=(hybrid_model is not None), label="Re-rank with Hybrid (if available)")
        useDual = gr.Checkbox(value=False, label="Use Dual-Pool Retrieval (Item+User CF)")
        w_user = gr.Slider(0.0, 3.0, value=1.0, step=0.1, label="Dual-Pool weight: User CF")
        w_item = gr.Slider(0.0, 3.0, value=1.0, step=0.1, label="Dual-Pool weight: Item CF")
        btn1 = gr.Button("Recommend")
        out1 = gr.Dataframe(headers=["ISBN","Title","Author","Year","Image"], wrap=True)

        def rec_for_user(user_id, topN, K_item, useH, useDual, w_user, w_item):
            if user_id is None: 
            ...
            if useDual:
                base = dual_pool_candidates(user_id, pool_item=max(3*topN,150), pool_user=max(3*topN,150),
                                            K_item=K_item, K_user=K_USER_DEFAULT, w_item=w_item, w_user=w_user)
            else:
                base = recommend_items_for_user(user_id, N=max(3*topN,100), K=K_item)
            final = rerank_with_hybrid(user_id, base, topN) if useH else base[:topN]
            return to_table(final)

        btn1.click(fn=rec_for_user, inputs=[user, topN, K_item, useH, useDual, w_user, w_item], outputs=out1)

    with gr.Tab("🧭 Similar Items (ISBN)"):
        isbn_in = gr.Textbox(label="ISBN from dataset")
        btn2 = gr.Button("Find Similar")
        out2 = gr.Dataframe(headers=["ISBN","Title","Author","Year","Image"], wrap=True)

        def similar_items(isbn, topN=TOPN_DEFAULT):
            if not isbn or isbn not in isbn2ix:
                return pd.DataFrame(columns=["ISBN","Title","Author","Year","Image"])
            ...
            isbns = [ix2isbn[i] for i in nbrs][:topN]
            return to_table(isbns)

        btn2.click(fn=similar_items, inputs=[isbn_in], outputs=out2)
